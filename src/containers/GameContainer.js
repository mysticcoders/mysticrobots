...
export const GameContainer = ({ goalIndex, goalColor, r, g, b, y }) => {
    ...
    useEffect(() => {
        history.replace(
            `/puzzle?goalIndex=${metadata.goalIndex}&goalColor=${metadata.goalColor}&r=${metadata.r}&g=${metadata.g}&b=${metadata.b}&y=${metadata.y}`
        )
    }, [metadata, history])
            ...
            <Column>
                {status === 'WIN' && (
                    <Notification color="success">
                        Congratulations! You solved the grid in {moveHistory.length} moves!{' '}
                        <Button onClick={() => dispatch(actions.setupBoard({}))}>Refresh</Button>
                    </Notification>
                )}
                    ...
                    <Column align="left">
                        {moveHistory.map((entry, idx) => (
                            <React.Fragment key={idx}>
                                {entry.direction === 'UP' && <FaArrowUp size="1.5em" style={{ color: entry.robot.toLowerCase() }} />}
                                {entry.direction === 'DOWN' && <FaArrowDown size="1.5em" style={{ color: entry.robot.toLowerCase() }} />}
                                {entry.direction === 'LEFT' && <FaArrowLeft size="1.5em" style={{ color: entry.robot.toLowerCase() }} />}
                                {entry.direction === 'RIGHT' && <FaArrowRight size="1.5em" style={{ color: entry.robot.toLowerCase() }} />}
                            </React.Fragment>
                        ))}
                    </Column>
                </Column.Group>
                ...
                <Level>
                    <Level.Item align="left">
                        <Button className="direction-button" onClick={() => dispatch(actions.moveUp())}>
                            <Icon size="medium">
                                <FaArrowAltCircleUp />
                            </Icon>
                        </Button>
                        <Button className="direction-button" onClick={() => dispatch(actions.moveDown())}>
                            <Icon size="medium">
                                <FaArrowAltCircleDown />
                            </Icon>
                        </Button>
                        <Button className="direction-button" onClick={() => dispatch(actions.moveLeft())}>
                            <Icon size="medium">
                                <FaArrowAltCircleLeft />
                            </Icon>
                        </Button>
                        <Button className="direction-button" onClick={() => dispatch(actions.moveRight())}>
                            <Icon size="medium">
