...
export default function (state = initialState, action) {
    switch (action.type) {
        ...
        case types.MOVE_SUCCESS:
            ...
            const lastMove = state.history.length && state.history[state.history.length - 1]
            const shouldUndo =
                lastMove.robot === action.payload.robot &&
                ((lastMove.direction === 'UP' && action.payload.direction === 'DOWN') ||
                    (lastMove.direction === 'DOWN' && action.payload.direction === 'UP') ||
                    (lastMove.direction === 'LEFT' && action.payload.direction === 'RIGHT') ||
                    (lastMove.direction === 'RIGHT' && action.payload.direction === 'LEFT'))

            let history
            if (shouldUndo) {
                history = state.history.slice(0, state.history.length - 1)
            } else {
                history = [...state.history, { direction: action.payload.direction, robot: action.payload.robot }]
            }
...
export function* setupBoard({ payload }) {
    ...
    const corners = Object.values(grid).filter(
        (element) =>
            ...
            element === WALL.SOUTH_EAST
    )

    const goalIndex =
        payload.goalIndex >= 0 && payload.goalIndex < corners.length ? payload.goalIndex : randomIntFromInterval(0, corners.length - 1)
    const randomCorner = corners[goalIndex]

    const goals = Object.values(GOAL)

    const goalColorIndex =
        payload.goalColor >= 0 && payload.goalColor < goals.length ? payload.goalColor : randomIntFromInterval(0, goals.length - 1)
    ...
    yield put({
        ...
        payload: { goalIndex, goalColor: goalColorIndex, r: rIndex, g: gIndex, b: bIndex, y: yIndex },
    })
    ...
    if (y < Y_MAX) {
        ...
        while (newY <= Y_MAX) {
            ...
            if (
                !done &&
                newY === y &&
                (newCell.walls === WALL.SOUTH || newCell.walls === WALL.SOUTH_EAST || newCell.walls === WALL.SOUTH_WEST)
            ) {
                done = true
            }
    ...
    if (x < X_MAX) {
        ...
        while (newX <= X_MAX) {
            ...
            if (
                !done &&
                x !== newX &&
                (newCell.walls === WALL.WEST || newCell.walls === WALL.SOUTH_WEST || newCell.walls === WALL.NORTH_WEST)
            ) {
...
export function* moveUp() {
    ...
    if (up && up.length > 0) {
        ...
        yield put({
            ...
            payload: { oldX: x, oldY: y, newX: moveCoord.x, newY: moveCoord.y, robot: robot, direction: 'UP' },
        })
...
export function* moveDown() {
    ...
    if (down && down.length > 0) {
        ...
        yield put({
            type: types.MOVE_SUCCESS,
            payload: { oldX: x, oldY: y, newX: moveCoord.x, newY: moveCoord.y, robot: robot, direction: 'DOWN' },
        })
        yield call(updateRobotPath, { payload: robot })
    }
}
...
export function* moveLeft() {
    ...
    if (left && left.length > 0) {
        ...
        yield put({
            type: types.MOVE_SUCCESS,
            payload: { oldX: x, oldY: y, newX: moveCoord.x, newY: moveCoord.y, robot: robot, direction: 'LEFT' },
        })
        yield call(updateRobotPath, { payload: robot })
    }
}
...
export function* moveRight() {
    ...
    if (right && right.length > 0) {
        ...
        yield put({
            type: types.MOVE_SUCCESS,
            payload: { oldX: x, oldY: y, newX: moveCoord.x, newY: moveCoord.y, robot: robot, direction: 'RIGHT' },
        })
        yield call(updateRobotPath, { payload: robot })
    }
}
